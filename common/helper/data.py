...
def write_pandas_parquet_to_s3(data, bucket_name: str, s3_file_name: str, tmp_file_name='data.parquet'):
    data_df = pd.DataFrame(data)
    table = pa.Table.from_pandas(data_df)
    pq.write_table(table, tmp_file_name)
    # upload to s3
    s3 = boto3.client("s3")
    try:
        s3.upload_file(tmp_file_name, bucket_name, s3_file_name)
    except Exception as e:
        print(f'Upload failed causes: {e}')


def get_sheet_id_from_google_sheet(url: str):
    print()
